...
class CachedGenieLoadTestUser(User):
    ...
    def on_start(self) -> None:
        ...
        logger.info(f"[User {self.user_id}] Initializing cached Genie service for space: {self.space_id}")
        ...
        logger.info(
            f"[User {self.user_id}] Cached Genie service initialized "
            f"(LRU capacity={LRU_CAPACITY}, TTL={CACHE_TTL}s, "
    ...
    def replay_conversation(self) -> None:
        ...
        logger.info(
            f"[User {self.user_id}] Replaying conversation: {conversation_id} - "
            f"\"{title_preview}\" ({len(messages)} messages)"
        )
        ...
        for i, msg in enumerate(messages):
            ...
            if i == 0:
                ...
                logger.info(
                    f"[User {self.user_id}]   Sending message {i+1}/{len(messages)}: "
                    f"after {wait_time_before:.1f}s \"{content_preview}\""
                )
            ...
            try:
                ...
                logger.info(
                    f"[User {self.user_id}]   Response received: {response_length} bytes "
                    f"in {response_time_secs:.2f}s [{cache_status}]"
                )
...
class CachedGenieSequentialUser(User):
    ...
    def on_start(self) -> None:
        ...
        logger.info(f"[SeqUser {self.user_id}] Initializing cached Genie service for space: {self.space_id}")
    ...
    def send_random_message(self) -> None:
        ...
        if self.last_message_time is None:
            ...
            logger.info(
                f"[SeqUser {self.user_id}] Sending message #{self.message_count}: "
                f"after {wait_time:.1f}s \"{content_preview}\""
            )
        ...
        try:
            ...
            logger.info(
                f"[SeqUser {self.user_id}]   Response received: {response_length} bytes "
                f"in {response_time_secs:.2f}s [{cache_status}]"
            )
        except Exception as e:
            exception = e
...
def on_test_start(environment: Any, **kwargs: Any) -> None:
    ...
    logger.info(f"  Lakebase Instance: {LAKEBASE_INSTANCE}")
    logger.info(f"  Warehouse ID: {WAREHOUSE_ID}")
    logger.info(f"  Cache TTL: {CACHE_TTL}s")
    logger.info(f"  Similarity Threshold: {SIMILARITY_THRESHOLD}")
    logger.info(f"  LRU Capacity: {LRU_CAPACITY}")
    logger.info("=" * 70)
